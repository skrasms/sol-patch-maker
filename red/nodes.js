...
*/
    ...
    //Aaron's function to get rid of every node
    function removeAllNodes() {
        // console.log("Saw this many links originally: ", links.length);
        while(nodes.length>0){
            RED.nodes.remove(nodes[0].id);
        }
        // console.log("See this many links left: ", links.length);
    }
	...
	function createExportableNodeSet(set) {
		...
		for (var n=0;n<set.length;n++) {
			//var node = set[n].n; //Original code that didn't work?
            //Aaron's note: I wanted to be able to pass this function the return from createCompleteNodeSet(), in which
            //case using set[n].n was breaking for multiple reasons. Even set[n] by itself fails to include the _def
            //parameter, and that omission breaks the call to convertNode. The solution for now is to use the id
            //from the set passed in as a reference into RED.nodes.node. I'm guessing the real solution is that
            //"set" should be something different coming in.
            var node = RED.nodes.node(set[n].id);
			...
			for (i=0;i<newNodes.length;i++) {
				...
				if (n.type !== "workspace" && n.type !== "tab") {
					...
					if (def && def.category == "config") {
                        ...
                        //Aaron mod: changed "new IDs" to come from Paul's createUniqueCppName. Also
                        //had to move this block to be after the "node._def = def" assignment in order
                        //for the unique name to recognize the block type.
                        if (createNewIds) {
							node.z = RED.view.getWorkspace();
							// node.id = getID();
                            node.id = createUniqueCppName(node);
						} else {
							node.id = n.id;
