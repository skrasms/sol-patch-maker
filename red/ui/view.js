...
RED.view = (function() {
	...
	function deleteSelection() {
		...
		if (moving_set.length > 0) {
			for (var i=0;i<moving_set.length;i++) {
				...
				for (var linkIndex=0; linkIndex < rmlinks.length; linkIndex++) {
					var link = rmlinks[linkIndex];
	...
	function calculateTextWidth(str) {
        ...
        lines.forEach(function(thisLine) {
            var lineWidth = calculateLineWidth(thisLine);
            if(lineWidth>w) {w = lineWidth}
        });
        return w;
	}
    
    function calculateLineWidth(str) {
	...
	function redraw() {
		...
		if (mouse_mode != RED.state.JOINING) {
			...
			nodeEnter.each(function(d,i) {
					...
					d.h = Math.max(node_height,(Math.max(d.outputs,d._def.inputs)||0) * 15, (d.numLines + 1) * 15);
				   ...
				   //node.append("rect").attr("class", "node-gradient-bottom").attr("rx", 6).attr("ry", 6).attr("height",30).attr("stroke","none").attr("fill","url(#gradient-bottom)").style("pointer-events","none");
                    ...
                    //Aaron's mods to add multiple text nodes for the label depending on how many lines (using \n)
                    //are in the label.
                    var labelForSizing = "";
                    if (d._def.label) {
                        if (typeof d._def.label == "function") {
                            labelForSizing = d._def.label.call(d);
                        } else {
                            labelForSizing = d._def.label;
                        }
                    }
                    var numLinesInLabel = labelForSizing.split(/\n/).length;
                    
                    for(var lineIndex=0; lineIndex<numLinesInLabel; lineIndex++){
			...
			node.each(function(d,i) {
					...
					if (d.dirty) {
						...
						if (d.resize) {
                            ...
                            d.numLines = l.split(/\n/).length;
							d.w = Math.max(node_width,calculateTextWidth(l)+(d._def.inputs>0?7:0) );
							d.h = Math.max(node_height,(Math.max(d.outputs,d._def.inputs)||0) * 15, 10 + (d.numLines + 1) * 15);
						}
                        ...
                        //This is the label drawing function that writes the text on the screen.
						thisNode.selectAll('text.node_label').text(function(d,i){
                                //The return here is what text is written to the screen.
                                var result = "";
                                if (d._def.label) {
									if (typeof d._def.label == "function") {
										result = d._def.label.call(d);
									} else {
										result = d._def.label;
									}
								}
							...
							.attr('y', function(d, i){
                                //i is the line number; lines are 15 tall.
                                return (d.h/2)-1 + i*15 - 6.5*(d.numLines-1);// -(d.numLines-1)*13 + i*13;
                            })
